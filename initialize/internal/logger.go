...
var (
	_       = New(log.New(ioutil.Discard, "", log.LstdFlags), config{})
	Default = New(log.New(os.Stdout, "\r\n", log.LstdFlags), config{
		SlowThreshold: 200 * time.Millisecond,
		LogLevel:      logger.Warn,
		Colorful:      true,
	})
	_ = traceRecorder{Interface: Default, BeginAt: time.Now()}
)
...
func (t *traceRecorder) Trace(_ context.Context, begin time.Time, fc func() (string, int64), err error) {
	t.BeginAt = begin
